...
pub fn distribute_community_reward(
    ...
    let (user_pda_index, user_slot_index) = ReferralStorage::decode_id(trigger_user_referral_id);
    let user_parent_id = {
    ...
    for depth in 0..COMMUNITY_MAX_TRAVERSE_DEPTH {
        ...
        let (pda_index, slot_index) = ReferralStorage::decode_id(current_id);
        if pda_index < 1 || pda_index > 9 {
            msg!("Invalid PDA index {} at depth {}", pda_index, depth);
            break;
        }
...
pub(crate) fn add_direct_reward_profit(
    ...
    if pda_index < 1 || pda_index > 9 {
        msg!("add_direct_reward_profit: invalid pda_index {} for id {}", pda_index, referral_id);
        return Ok(());
    }
    let storage_account = storage_accounts[(pda_index - 1) as usize];
    let mut storage_data = storage_account.try_borrow_mut_data()?;
...
pub(crate) fn add_team_reward_profit(
    ...
    let (pda_index, slot_index) = ReferralStorage::decode_id(referral_id);
    if pda_index < 1 || pda_index > 9 {
        msg!("add_team_reward_profit: invalid pda_index {} for id {}", pda_index, referral_id);
        return Ok(());
    }
    let storage_account = storage_accounts[(pda_index - 1) as usize];
    let mut storage_data = storage_account.try_borrow_mut_data()?;
...
/// Cross-week detection & refresh
///
/// If the current time has crossed into a new week, automatically:
...
pub(crate) fn maybe_refresh_week(
    ...
    if total_unclaimed > 0 {
        ...
        msg!("Node pool unclaimed {} -> root", total_unclaimed);
    }
